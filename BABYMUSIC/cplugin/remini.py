...
@Client.on_message(filters.command(["remini", "enhance"]) & filters.private & ~BANNED_USERS)
async def upscale_image(client: Client, message: Message):
    try:
        ...
        progress_msg = await message.reply_text("Enhancing your image, please wait...")
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    await message.reply_text(
                        f"Enhancing failed with status code {response.status}. Please try again later."
                    )
        ...
        # Ensure cleanup of the original image if it exists
